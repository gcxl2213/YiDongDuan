        ...
        HttpUtil.sendEmail(veriCode, type, new BaseObserver<DefaultResultBean<Object>>() {
            ...
            protected void onSuccess(DefaultResultBean<Object> objectDefaultResultBean) {
                ...
                if (objectDefaultResultBean.getCode().equals("20000")) {
                    ToastUtil.showMessage(LoginActivity.this, "发送成功", ToastUtil.LENGTH_LONG);
                } else {
            ...
            protected void onFailure(Throwable e, boolean isNetWorkError) {
                ...
                if (isNetWorkError)
                    ToastUtil.showMessage(LoginActivity.this, "请求失败，请检查网络", ToastUtil.LENGTH_SHORT);
                else
                    ToastUtil.showMessage(LoginActivity.this, "发送失败", ToastUtil.LENGTH_SHORT);
            }
        });
    }
    @OnClick(R.id.qq_go)
...
//    }
    ...
    private void switchType() {
        ...
        btSwitchType.setText(loginTypeVerifyCode ? "密码登录" : "验证码登录");
